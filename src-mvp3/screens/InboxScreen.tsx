...
export default function InboxScreen({
  ...
  const endSwipe = (event: React.PointerEvent<HTMLDivElement>, id: string, openTap = true) => { if (!swipe || swipe.id !== id) return; const action = Math.abs(swipe.offset) >= 52 ? swipe.action : null; setOpenSwipe(action ? { id, action } : null); setSwipe(null); if (!action && openTap && !swipeMoved.current) { suppressNextClick.current = true; onOpenConversation(id); } if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); };
