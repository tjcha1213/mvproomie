...
export default function InboxScreen({
  ...
  const beginSwipe = (event: React.PointerEvent<HTMLDivElement>, id: string) => {
    ...
    setSwipe({ id, startX: event.clientX, offset: 0, action: null });
  };
  ...
  const moveSwipe = (event: React.PointerEvent<HTMLDivElement>, id: string) => {
    ...
    const action = delta < 0 ? 'delete' : 'pin';
    setSwipe({ ...swipe, offset: Math.max(-88, Math.min(88, delta)), action });
  };
    ...
    return (
      <>
        ...
        <div className="inbox-chat-header">
          <button className="inbox-chat-back" type="button" onClick={onBackToList} aria-label="Back to chat list">
          ...
          {conversations.map((conversation) => {
            ...
            return (
              <div
                ...
                onPointerDown={(event) => { if (!(event.target as HTMLElement).closest('.inbox-swipe-action')) beginSwipe(event, conversation.id); }}
                onPointerMove={(event) => moveSwipe(event, conversation.id)}
                onPointerUp={(event) => { if (!(event.target as HTMLElement).closest('.inbox-swipe-action')) endSwipe(event, conversation.id); }}
                onPointerCancel={(event) => endSwipe(event, conversation.id, false)}
              >
                <div className="inbox-swipe-actions">
                  <button className="inbox-swipe-action inbox-pin-action" type="button" aria-label={conversation.pinned ? 'Unpin conversation' : 'Pin conversation'} onClick={(event) => { event.stopPropagation(); onTogglePinConversation(conversation.id); setOpenSwipe(null); }}>
                    <span className="inbox-action-icon">{conversation.pinned ? '★' : '☆'}</span>
                    <span>{conversation.pinned ? 'Unpin' : 'Pin'}</span>
                  </button>
                  <button className="inbox-swipe-action inbox-delete-action" type="button" aria-label="Delete conversation" onClick={(event) => { event.stopPropagation(); setOpenSwipe(null); onDeleteConversation(conversation.id); }}>
                    <span className="inbox-action-icon">×</span>
                    <span>Delete</span>
                  </button>
                </div>
                <button className="inbox-item" type="button" style={{ transform: `translateX(${offset}px)` }} onClick={() => {
                  if (suppressNextClick.current) { suppressNextClick.current = false; return; }
                  if (swipeMoved.current) { swipeMoved.current = false; return; }
                  if (isOpen) { setOpenSwipe(null); return; }
